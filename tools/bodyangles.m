...
%     i = find(max(wristZ));
    ...
    p.C = 36.4291;              % flywheel damping coefficient
	p.m = 554.6741;             % flywheel equivalent mass

  	p.Kcrm = 1313.8565;             % stiffness of cable and ratchet mechanism (N/m) (CHECK VALUE IN PARAMETER IDENTIFICATION)
 %	p.Kcrm = 0;             % stiffness of cable and ratchet mechanism (N/m) (CHECK VALUE IN PARAMETER IDENTIFICATION)

%    p.K =  17.5250 ;			% shock cord stiffness (N/m)
    %  p.K =  17.5250 ;			% shock cord stiffness (N/m)

    p.K =  20 ;			% shock cord stiffness (N/m)

    p.L0 = 0.6;                   % the wrist-sprocket distance at which shock cord has no force
